...
export default function VotacionesSeccionScreen() {
  ...
  const { id: seccionId } = useLocalSearchParams();
        ...
        {votaciones.length === 0 ? (
          ...
          votaciones.map((votacion) => (
            <TouchableOpacity
              key={votacion.id}
              style={[
                styles.tarjetaVotacion,
                votacion.estado === 'cerrada' && styles.tarjetaCerrada,
              ]}
              onPress={() => {
                if (votacion.estado === 'abierta') {
                  router.push(`/votar/${votacion.id}` as any);
                }
              }}
                ...
                <View style={styles.metadatos}>
                  <View style={styles.badge}>
                    <Text style={styles.badgeTexto}>
                      {obtenerEtiquetaMetodo(votacion.metodoVotacion)}
                    </Text>
                  </View>
                  {votacion.metodoVotacion === 'multiple' && votacion.maxOpciones && (
                    <View style={[styles.badge, styles.badgeSecundario]}>
                      <Text style={styles.badgeTextoSecundario}>
                        Máx: {votacion.maxOpciones}
                      </Text>
                    </View>
                  )}
                  <View
                    style={[
                      styles.badge,
                      votacion.estado === 'abierta' ? styles.badgeAbierta : styles.badgeCerrada,
                    ]}
                  >
                    <Text style={styles.badgeTexto}>
